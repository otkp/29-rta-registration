	...
	public PaymentGateWayResponse processVerify(PaymentTransactionDTO paymentTransactionDTO) {
		...
		try {
			...
			if (response.hasBody()) {
				...
				String decryptData = response.getBody().substring(
						response.getBody().indexOf("value=") + 7,
			...
			logger.error("No response from SBI Gateway for Application No [{}]",applicationNo);
			throw new BadRequestException("No respopnce from SBI Gateway for Application No [" + applicationNo + "]");

		} catch (RestClientException rce) {
			logger.debug("Debug is at While Connecting to SBI Gateway [{}]",rce);
			logger.error("Error while Connecting to SBI Gateway [{}]",rce.getMessage());

...
import org.epragati.ruleengine.service.ChalanDetailsService;
...
public class PayUGateWay implements PaymentGateWay {
	...
	public String getPayURequestParameter(TransactionDetailVO transactionDetailVO, Map<String, String> gatewayDetails) {
		...
		try {
			if (transactionDetailVO.getModule().equalsIgnoreCase(ServiceEnum.SPNR.getCode())
				...
				transactionDetailVO
				.setSucessUrl(gatewayDetails.get(GatewayTypeEnum.PayUParams.CITIZEN_SUCESS_URL.getParamKey()));
	...
	public PaymentGateWayResponse processVerify(PaymentTransactionDTO paymentTransactionDTO) {
		...
		try {
			ResponseEntity<PayUVerifyResponse> response = restTemplate.postForEntity(
			...
			if (response.hasBody()) {
				...
				if (CollectionUtils.isNotEmpty(result)) {
					...
					// GateWayDTO gatewayValue =
					// gatewayDao.findByGateWayType(GatewayTypeEnum.PAYU);
					Map<String, String> gatewayDetails = gatewayValue.getGatewayDetails();
					String payUKey;
					String payUSalt;
					...
					paymentGateWayResponse.setAppTransNo(payUResponse.getUdf1());// applicant
																					// from
																					// id
					paymentGateWayResponse.setModuleCode(payUResponse.getUdf2());// Service
																					// Code
					paymentGateWayResponse.setTransactionNo(payUResponse.getTxnid());
					paymentGateWayResponse.setBankTranRefNumber(payUResponse.getBank_ref_num());
